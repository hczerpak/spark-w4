...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val theRest = columnNames.tail.map( name => StructField(name, DoubleType, nullable = false))

    StructType(firstField :: theRest)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def startingWithAny(name:String, prefixes: List[String]): Boolean = prefixes.exists(prefix => name.startsWith(prefix))

    (
      columnNames.filter(c => startingWithAny(c, primaryNeeds)).map(name => new Column(name)),
      columnNames.filter(c => startingWithAny(c, work)).map(name => new Column(name)),
      columnNames.filter(c => startingWithAny(c, leisure) && !startingWithAny(c, work) && !startingWithAny(c, primaryNeeds)).map(name => new Column(name))
    )
  }
